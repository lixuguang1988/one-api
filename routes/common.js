...
router.post('/upload', jwtVerify, upload.single('file'), (req, res) => {
  ...
  res.status(200).json({code: 200, message: 'File uploaded successfully', data: {
    ...req.file,
    url: getUrl(req.file)
  }});
});
...
router.post('/upload-multiple', upload.array('files', 5), (req, res) => {
  ...
  res.status(200).json({code: 200, message: 'Files uploaded successfully', data: req.files.map(item => {
    return {
      ...item,
